...
- **Serialization (직렬화)**: "데이터 포장". 구조체를 바이트 배열로 변환하는 과정.
- **Deserialization (역직렬화)**: "데이터 개봉". 바이트 배열을 다시 구조체로 복원하는 과정.
- **Endianness**: "순서 규칙". 데이터를 저장하는 바이트 순서 (Big-endian vs Little-endian).
