...
- **Layered Architecture (계층형 아키텍처)**: "샌드위치 구조". 소프트웨어를 역할별로 층층이 쌓아 올린 구조입니다. (네트워크층, 로직층, 데이터층 등)
- **Dependency Rule (의존성 규칙)**: "위에서 아래로만". 상위 계층은 하위 계층을 써도 되지만, 하위 계층은 상위 계층을 알면 안 된다는 원칙입니다.
- **Skeleton Code (스켈레톤 코드)**: "뼈대". 세부 내용은 비어있지만 전체적인 구조와 흐름을 파악할 수 있는 기초 코드입니다.
...
5. `main.cpp`에서 서버가 시작되고 종료되는 전체 흐름을 확인하세요.
